...
export default class Backend extends Command {
    ...
    public static summary = `Execute command in the ${chalk.cyan('backend')} container. Empty argument starts a shell session`;
    ...
    public async run (): Promise<void> {
        this.log(chalk.yellow(
            "You're working inside the 'backend' application container of this project.",
        ));

        if (this.argv.length) {
            execSync(
                `docker exec -it mission_control_backend bash -c "${this.argv.join(' ')}";`,
                { stdio: 'inherit' },
            );
        ...
        execSync(
            'docker exec -it mission_control_backend bash;',
            { stdio: 'inherit' },
        );
    }
}
