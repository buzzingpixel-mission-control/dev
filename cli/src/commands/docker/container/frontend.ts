...
export default class Frontend extends Command {
    ...
    public static summary = `Execute command in the ${chalk.cyan('frontend')} container. Empty argument starts a shell session`;
    ...
    public async run (): Promise<void> {
        this.log(chalk.yellow(
            "You're working inside the 'frontend' application container of this project.",
        ));

        if (this.argv.length) {
            execSync(
                `docker exec -it mission_control_frontend bash -c "${this.argv.join(' ')}";`,
                { stdio: 'inherit' },
            );
        ...
        execSync(
            'docker exec -it mission_control_frontend bash;',
            { stdio: 'inherit' },
        );
    }
}
